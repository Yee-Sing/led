...
def preprocess_data(data, If_label_ts=False):
    ...
    if If_label_ts:
        ...
        # spiking value for event representation
        data[data != 0] = 1
    ...
    def get_train_data_files(self, data_dir):
        ...
        for scene_dir in scene_dirs:
            ...
            if os.path.isdir(scene_path):
                ...
                files = files[self.start_index: self.start_index + self.time_steps]  # select certain quantity file
    ...
    def get_train_data_files(self, data_dir):
        ...
        for scene_dir in scene_dirs:
            ...
            if os.path.isdir(scene_path):
                files = os.listdir(scene_path)
                files = sorted(files, key=self.sort_by_number)
                files = files[self.start_index: self.start_index + self.time_steps]
                files = [os.path.join(scene_path, file) for file in files]
                train_data_files.append(files)
        return train_data_files
...
def compute_loss(original, prediction, target, thres_prediction, thres_target):
    ...
    # calculation the denoising accuracy
...
def train_denoisingsnn(args):
    ...
    # define the your dataset
    sample_sequence = 10  # files number in each subfolder
    time_sequence = 10  # snn time step
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for time_slice in range(sample_sequence // time_sequence):
            ...
            for data_batch, label_batch, label_ts_batch, datanames_batch, labelnames_batch in train_loader:
                ...
                states = None  # It is necessary to reset the states for each batch after it is loaded.
                ...
                # After optimizing the parameters once, it is necessary to reset
                # the state of the network and clear the "memory" of the SNN neurons
