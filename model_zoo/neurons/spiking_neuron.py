...
from spikingjelly.activation_based import neuron, surrogate, base, layer
import math
import numpy as np
...
class BaseNode(base.MemoryModule):
    def __init__(self, v_threshold: float = 1., v_reset: float = 0.,
        ...
        if v_reset is None:
            ...
            self.register_memory('spike', 0.)#只是初次计算使用到？
        else:
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 输出点火率
        # print('self.spike.shape=', self.spike.shape)
        # nonzero_count = self.spike.cpu().nonzero().size(0)
        # total_count = self.spike.cpu().numel()
        # spike_rate=nonzero_count/total_count
...
class MpNode(base.MemoryModule):
    ...
    def forward(self, x: torch.Tensor, last_mem: torch.Tensor):
        if last_mem is None:
            ...
            self.v = last_mem#训练时会引发错误的原码self.register_memory('v', last_mem)#调用 self.v 来引用 last_mem。也就是说，self.v 就是 last_mem 在当前模块中的别名
            self.neuronal_charge(x)#根据输入 x 继续充电计算self.v，最后返回更新后的膜电位 self.v
        return self.v
...
class LIFNode(BaseNode):#继承了类‘BaseNode’,点火和重置都写在BaseNode
    def __init__(self, tau: float = 2., v_threshold: float = 1.,
                 v_reset: float = 0., surrogate_function: Callable = surrogate.Sigmoid(),
                 detach_reset: bool = False):
