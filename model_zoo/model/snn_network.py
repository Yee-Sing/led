...
class Spiking_residualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out + residual#会引发错误的原代码out += residual
        out = self.lif(out)
        return out
...
class TemporalFlatLayer_concat(nn.Module):
    def __init__(self, tau=2.0, v_reset=None):
        ...
        self.conv2d = nn.Conv2d(64, 1, 1, bias=False)#原始为nn.Conv2d(64, 1)
        self.norm_layer = nn.BatchNorm2d(1)#原始为nn.BatchNorm2d(1)
        self.activation = MpLIFNode(v_threshold=float('Inf'), tau = tau, v_reset=v_reset, surrogate_function=surrogate.ATan())
        #self.activation = MpIFNode(v_threshold=float('Inf'), v_reset=v_reset, surrogate_function=surrogate.ATan())
        #self.activation = Mp_ParametricLIFNode(v_threshold=float('Inf'), v_reset=v_reset, surrogate_function=surrogate.ATan()) 
...
class ThresAdaptive_Module(nn.Module):
    def __init__(self, activation_type, v_threshold, v_reset, tau):
        super(ThresAdaptive_Module, self).__init__()
    ...
    def forward(self, on_img, prev_mem_states):
        ...
        outputs = self.sigmoid(membrane_potential)
...
class EVSNN_LIF_final(BasicModel):
    ...
    def __init__(self, kwargs={}):
        ...
        v_reset = kwargs['v_reset']
        tau = kwargs['tau']
        # print('v_reset=', v_reset) # v_reset= 0.0
        #header
        self.static_conv = nn.Sequential(
            nn.Conv2d(1, 32, kernel_size=5, padding=2, bias=False),#原本为单通道 nn.Conv2d(1, 32)
            nn.BatchNorm2d(32),
            LIFNode(v_threshold=v_threshold, tau=tau, v_reset=v_reset, surrogate_function=surrogate.ATan()),
        )
    ...
    def forward(self, on_img, prev_mem_states):
        # print('on_img.shape=', on_img.shape)# on_img.shape= torch.Size([batch, channel, H, W])
        #原始for i in range(on_img.size(1)):
        x = on_img#原始[:, i, :, :].unsqueeze(dim=1)# x.shape= torch.Size([batch, channel, H, W])
        ...
        u1 = self.up1(torch.cat([r1, x3], 1))#在channel维度级联
        # if u1.shape[2] != x2.shape[2]:#强行防止维度不一致
        #     diff = u1.shape[2] - x2.shape[2]
        #     x2 = torch.cat([x2, x2[:, :, -diff:, :]], dim=2)
        u2 = self.up2(torch.cat([u1, x2], 1))
        u3 = self.up3(torch.cat([u2, x1], 1))
...
class EDSNN_LIF_ADAPTIVE_final(BasicModel):
    ...
    def __init__(self, kwargs={}):
        ...
        self.thresadaptive = ThresAdaptive_Module(activation_type=activation_type, v_threshold=v_threshold, v_reset=v_reset, tau=tau)
    ...
    def forward(self, on_img, prev_mem_states):
        ...
        thres_map = self.thresadaptive(x, None)
        ...
        return outputs, thres_map

class EDSNN_LIF_contrast(BasicModel):
